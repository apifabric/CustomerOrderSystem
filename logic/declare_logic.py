...
def declare_logic():
    ...
    # Customer's balance must not exceed their credit limit.
    Rule.constraint(validate=Customer,
                as_condition=lambda row: row.balance <= row.credit_limit,
                error_msg="Customer balance ({row.balance}) exceeds credit limit ({row.credit_limit})")

    # Calculate customer's balance as the sum of their orders' total amounts that have not been shipped.
    ...
    # Copy the unit price from product to item.
